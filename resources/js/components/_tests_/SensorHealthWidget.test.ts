...
describe('combined sensor health widget', () => {
    ...
    it('fetches and renders both authorized health sources independently', async () => {
        ...
        expect(mocks.useIntervalFn).toHaveBeenNthCalledWith(1, expect.any(Function), 20_000, { immediateCallback: true });
        expect(mocks.useIntervalFn).toHaveBeenNthCalledWith(2, expect.any(Function), 20_000, { immediateCallback: true });
    });
    ...
    it('keeps one source visible when the other source fails', async () => {
        ...
        expect(wrapper.text()).toContain('Roof');
        expect(wrapper.get('time').attributes('datetime')).toBe('2026-07-25T11:59:30.000Z');
    });
});
