...
it('authorizes when user can view widget', function () {
    ...
    $user->shouldReceive('can')->with('peoplecount.widgets.activeareacounts')->andReturn(true);
...
it('does not authorize when user cannot view widget', function () {
    $user = Mockery::mock(User::class);
    $user->shouldReceive('can')->with('peoplecount.widgets.activeareacounts')->andReturn(false);
